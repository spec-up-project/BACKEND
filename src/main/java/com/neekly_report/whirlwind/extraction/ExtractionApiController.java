...
public class ExtractionApiController {
    ...
    @Operation(summary = "이미지 OCR 후 일정/할일 추출",
            description = "이미지를 OCR로 텍스트 변환 후 일정과 할일을 추출합니다")
    @PostMapping("/ocr")
    public ResponseEntity<ApiResponseDto<ExtractionDto.Response.ExtractionResult>> extractFromImage(
            @RequestParam("image") MultipartFile image,
            @RequestParam(value = "language", defaultValue = "kor+eng") String language,
            @AuthenticationPrincipal UserDto.UserDetail userDetail) {

        log.info("이미지 OCR 추출 요청 - 사용자: {}, 파일크기: {}KB",
                userDetail.getTUserUid(), image.getSize() / 1024);

        ExtractionDto.Request.ImageExtractionRequest request =
                ExtractionDto.Request.ImageExtractionRequest.builder()
                        .image(image)
                        .language(language)
                        .build();

        ExtractionDto.Response.ExtractionResult result =
                extractionService.extractFromImage(request, userDetail.getTUserUid());

        return ResponseEntity.ok(ApiResponseDto.success(result, "이미지 OCR 추출이 완료되었습니다."));
    }
